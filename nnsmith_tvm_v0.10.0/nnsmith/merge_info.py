...
for iteration_count_hhh in range(1):
    ...
    normal_random_elements = random.sample(range(models_count), 90000)
    ...
    error_accuracy_random_elements = random.sample(range(models_count), 0)
    for iteration_count in error_accuracy_random_elements:
    ...
    models_path = 'error_other_only_2'
    models_count = 0
    for sub_dir in os.listdir(models_path):
        if sub_dir.startswith("model_") and sub_dir[6:].isdigit():
            models_count += 1
    print('The count of error other models: ',models_count)
    error_other_random_elements = random.sample(range(models_count), 11)
    for iteration_count in error_other_random_elements:
        model_path_dic = "error_other_only_2/model_{}".format(iteration_count)
        model_path = "{}/model.onnx".format(model_path_dic)
        data_info_json_filename = '{}/data_info.json'.format(model_path_dic)
        with open(data_info_json_filename, 'r') as jsonfile:
            data_info = json.load(jsonfile)
    ...
    json_filename = './tvm-v0.10.0-testdata_0_11/demo_test_data_info_{}.json'.format(iteration_count_hhh)
    with open(json_filename, 'w') as jsonfile:
        json.dump(shuffled_data_info_list, jsonfile, indent=4)
    
    json_filename = './tvm-v0.10.0-testdata_0_11/demo_test_time_info_{}.json'.format(iteration_count_hhh)
    with open(json_filename, 'w') as jsonfile:
