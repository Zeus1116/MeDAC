...
# 定义源文件夹和目标文件夹的路径
source_folder = "error_other_2"
target_folder = "error_op_2"
    ...
    if os.path.isdir(model_folder_path):
        ...
        if os.path.isfile(err_log_path):
            with open(err_log_path, 'r') as file:
                contents = file.read()
                # 检查文件内容是否包含指定字符串
                if "Unsupported operator" in contents or "unknown intrinsic Op" in contents or "NOT_IMPLEMENTED" in contents:
